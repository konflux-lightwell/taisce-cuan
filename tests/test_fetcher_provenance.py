...
def test_rhtl_advertised_origin_binds_raw_provenance(tmp_path: Path):
    ...
    fetcher = SdistSourceFetcher(
        "https://rhtl.example/simple", "https://pypi.example/pypi", client=client
    )
...
def test_fetcher_resets_state_between_calls(tmp_path: Path):
    ...
    rhtl_index = json.dumps(
        {
            "files": [
                {
                    "filename": "demo-1.0.tar.gz",
                    "url": archive_a_url,
                    "hashes": {"sha256": hashlib.sha256(archive_a).hexdigest()},
                    "size": len(archive_a),
                    "provenance": None,
                }
            ]
        ...
        "urls": [
            {
                ...
                "url": archive_b_url,
                "digests": {"sha256": hashlib.sha256(archive_b).hexdigest()},
                "size": len(archive_b),
            }
        ]
    }
...
@pytest.mark.parametrize(
    "invalid_prov",
    ["", "   ", "http://insecure.example/prov.json", False, {"nested": "dict"}],
)
...
def test_git_mirror_advertised_rhtl_evidence_digest_validation(
    ...
    origin = {
        ...
        "acquired": {
            "registry": "rhtl",
            "url": "https://rhtl.example/demo.tar.gz",
            "sha256": archive_sha,
            "size": len(archive),
            "package": "demo",
            "version": "1.0",
            "path": "downloads/demo-1.0.tar.gz",
        },
        "provenance": {
            "mode": "rhtl",
            "status": "advertised",
            ...
            "reference": "provenance.pep740.json",
            "sha256": prov_sha,
            "url": "https://rhtl.example/provenance.json",
            "rhtl": {
                "status": "advertised",
                "evidence": {
                    "path": "rhtl-index.pep691.json",
                    "sha256": index_sha,
                    "status": 200,
                    "url": "https://rhtl.example/simple/demo/",
                    "reason": None,
                },
            },
        },
    }
    (carrier / "source-origin.json").write_text(
        ...
        json.dumps(
            {
                ...
                "source_origin_sha256": hashlib.sha256(
                    (carrier / "source-origin.json").read_bytes()
                ).hexdigest(),
    ...
    with pytest.raises(
        ValueError,
        match="rhtl-index.pep691.json does not match source-origin provenance digest",
    ):
        publisher.publish_source(
            source_path=normalized_sdist,
            package="demo",
            version="1.0",
