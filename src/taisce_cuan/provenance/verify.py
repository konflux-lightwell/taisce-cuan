...
def locate_source_artifact_paths(
    ...
    if not origin_file.is_file():
        raise ArtifactError("source-origin.json is required beside the normalized sdist")
    if not transformation_file.is_file():
        raise ArtifactError("sdist-transformation.json is required beside the normalized sdist")
...
def verify_relative_artifact_path(root: Path, relative_path_str: str, label: str) -> Path:
    ...
    if not (resolved == resolved_root or resolved.is_relative_to(resolved_root)):
        raise ArtifactError(f"Dangerous path traversal detected in {label}: {relative_path_str}")
...
def verify_sha256(path: Path, expected: str, label: str) -> str:
    ...
    if actual.lower() != expected.lower():
        ...
        raise ProvenanceVerificationError(
            f"{label} digest mismatch: expected {expected.lower()}, got {actual.lower()}"
        )
    return actual.lower()
...
def verify_acquired_source(
    ...
    if acquired_path_str and Path(acquired_path_str).name != archive_path.name:
        raise ProvenanceVerificationError("source-origin acquired path does not match carried original archive")
...
def verify_transformation(
    ...
    """
    Validate transformation input/output/source-origin digests and normalized-output basename.
    ...
    if not all(
        ...
        raise ProvenanceVerificationError(
            "sdist-transformation.json must contain input, output, and source-origin digests"
        )

    acquired_digest = compute_sha256(acquired.sdist)
    if transform_input_digest.lower() != acquired_digest.lower():
        raise ProvenanceVerificationError("sdist transformation input does not match acquired digest")
...
def verify_upstream_evidence(
    ...
    """
    Enforce PyPI/RHTL evidence-state matrix, hash only applicable opaque evidence file(s).
    ...
    if route == SourceRoute.RHTL:
        ...
        if is_advertised or status == "advertised":
            ...
            if not pep740_file.is_file():
                raise ProvenanceVerificationError("Advertised RHTL provenance file is missing: provenance.pep740.json")
            if pep740_digest:
...
def copy_verified_evidence(evidence_files: tuple[Path, ...], destination_dir: Path) -> None:
    """Copy only the returned, already verified files; does not rediscover optional files."""
    destination_dir.mkdir(parents=True, exist_ok=True)
    for src in evidence_files:
        if not src.is_file():
            raise ProvenanceVerificationError(f"Verified evidence file does not exist to copy: {src}")
...
def verify_lightwell_attestations(
    ...
    *,
    ...
    if not meta_dsse.is_file():
        raise ProvenanceVerificationError("Signed publication missing .lightwell/metadata.dsse.json")

    prov_dsse = lightwell_dir / "provenance.dsse.json"
    if route == SourceRoute.PYPI:
        if not prov_dsse.is_file():
            raise ProvenanceVerificationError("Signed PyPI publication missing .lightwell/provenance.dsse.json")
...
def verify_normalized_source_artifact(
    ...
    *,
    ...
    """
    carrier_root, sdist_path, origin_file, transformation_file = locate_source_artifact_paths(
