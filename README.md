...
2. **`push`**: Takes a source archive, unpacks it into a canonical repository tree, records `.lightwell/metadata.json` (SLSA Build Provenance inventory with git tree SHA), commits the version as a rootless snapshot on a version-derived stream branch (`stream/{major}.{minor}`), tags `<canonical-pkg>/<version>`, and pushes to the Git forge.
